...
renderGauge({
  rate <- round((sum(jail.load$Gender == "Male")) / length(jail.load$Gender) * 100, 1)
...
renderValueBox({
  ja <- jail.load
  num <- round(mean(ja$Current.Age, na.rm = T), 0)
  valueBox("Avg Current Age", value = num, color = "green")
})
```
