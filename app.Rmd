...
```{r}
#plot of age at booking by race
renderPlotly({
    dat <- jaInput()
    ggplot(data = dat, aes(x = Age.at.Booking , fill = Race)) + 
        geom_histogram()+
        labs(x = "Age at Booking", title = "Age at Booking per Race") +
        guides(color = FALSE)
  })
```
...
renderPlotly({
    dat <- jaInput()
    ggplot(data = dat, aes(x = Age.at.Booking)) + 
        geom_area(stat = "bin", aes(fill = dat$Gender), position = position_stack(reverse = T))+
