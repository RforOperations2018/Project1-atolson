...
ckanUniques <- function(id, field) {
  url <- paste0("http://https://data.wprdc.org/api/3/action/datastore_search_sql?sql=SELECT%20DISTINCT(%22", field, "%22)%20from%20%22", id, "%22")
  c(ckanSQL(URLencode(url)))
}

races <- sort(c("Black", "White", "Hispanic", "Asian", "American Indian", "Unknown"))

jaInput <- reactive({
  #building API query with date filter
  url <- paste0("http://https://data.wprdc.org/api/3/action/datastore_search_sql?sql=SELECT%20Date%2C%20Gender%2C%20Race%2C%20%22Age%20at%20Booking%22%20FROM%20%2266cdcd57-6c92-4aaa-8800-0ed9d8f03e22%22%20WHERE%20Date%20>=%20%27", input$DateSelect[1], "%27%20AND%20Date%20<=%20%27", input$DateSelect[2], "%27")
...
jaRInput <- reactive({
  #building API query with date filter
  url <- paste0("http://https://data.wprdc.org/api/3/action/datastore_search_sql?sql=SELECT%20Date%2C%20Gender%2C%20Race%2C%20%22Age%20at%20Booking%22%20AS%20Age%2Eat%2EBookingFROM%20%2266cdcd57-6c92-4aaa-8800-0ed9d8f03e22%22%20WHERE%20Date%20>=%20%27", input$DateSelect[1], "%27%20AND%20Date%20<=%20%27", input$DateSelect[2], "%27")
...
```{r}
DT::renderDataTable({
  subset(jaInput(), select = c(Date, Gender, Race, Age.at.Booking))
})
```
